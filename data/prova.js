...
function aggiungiEtichetteAraldi(){
    if(window.location.href=="http://extremelot.leonardo.it/proc/chat_middle.asp"){
        try{
             mappaturaDigitale();
             $('img', window.parent.testo.document).each(function(){
...
$(document).ready(function(){
     ...
     if(($("html").find("title").text()=="ExtremeLOT - Il Regno Fantasy Virtuale")){
        ...
        aggiungiMapButton();
         ...
         $("html").append("<div id=\"notifica\" style=\"position:fixed;top:40px;left:20px;width:210px;height:100;background-color:#DEB887;overflow:hidden;text-align:left;font-size:70%;\">***</div>");
         $("#notifica").hide();
         ...
         //$("#amici").draggable();//TODO MAYBE IT BREAKS EVERYTHING
...
function aggiungiConfig(){
	...
	$("#configbutton").click(function(){
		...
		//alert("apriti sesamo "+$.ui.version);
		$("#configdialog").show();
    });
}
...
function aggiungiDialog(){
    var dialog="<div id=\"configdialog\" title=\"impostazioni\" style=\"position:fixed;top:200px;left:400px;background-color:#DEB887;overflow:visible;padding:10px 10px 10px 10px;box-shadow: 10px 10px 5px #888888;-moz-box-shadow: 10px 10px 5px #888888;border-style:solid;\" >"+ 
    "<label>E-mail</label><br />"+
    "<input id=\"mail\" type=\"text\" value=\"\" style=\"width:100%;\" /><br />"+
    "<input id=\"yessound\" type=\"checkbox\" name=\"yessound\" value=\"true\">Abilita Suoni Missive<br />"+
    "<input id=\"yesnotifica\" type=\"checkbox\" name=\"yesnotifica\" value=\"true\">Abilita Notifiche Missive<br />"+
    "<hr />"+
    ...
    "<div id=\"bottoni\" style=\"float:right;\">"+
    ...
    "</div>";
     ...
     var h=$(window).height();
     var w=$(window).width(); 
     var hf= $("#configdialog").height();
     var wf= hf*1.618;
     $("#configdialog").width(wf);
     $("#configdialog").css("top",(h/2-hf/2)+"px");
     $("#configdialog").css("left",(w/2-wf/2)+"px");
     $("#configdialog label").css("font-weight","bold");
     $("#configdialog label").css("padding-top","3px");
     
     $("#configdialog").hide();
     //$("#configdialog").draggable();//TODO MAYBE IT BREAKS EVERYTHING
     ...
     $("#submitmail").click(function(){
        ...
        addLC("yessound",$("#yessound").is(':checked'));
        addLC("yesnotifica",$("#yesnotifica").is(':checked'));
        $("#configdialog").hide();
        var listaAmici=checkLC("amici");
        preparaBoxAmici(listaAmici,"amici");
    });
    ...
    $("#yessound").attr('checked',checkLC("yessound"));
    $("#yesnotifica").attr('checked',checkLC("yesnotifica"));
...
function scaricaMex(id_mex){
    $.get('http://extremelot.leonardo.it/proc/posta/apriposta.asp?msg='+id_mex, function (data) {
        ...
        mex_parsed+="\n\nFirmato: "+mittente;
        mostraNotificaMessaggio(mex_parsed,id_mex);
...
function mostraNotificaMessaggio(mex,id_mex){
	var ys=checkLC("yessound");
	var yn=checkLC("yesnotifica");
	if(yn=="true"){
		$("#notifica").html(mex.replace(/\n/g,'<br />'));
		$("#notifica").show();
	}
	if(ys=="true"){
		var audio = document.getElementById('notificaSonora');
		audio.play();
	}
	$("#notifica").click(function(){
		window.open('http://extremelot.leonardo.it/proc/posta/apriposta.asp?msg='+id_mex);
		if(yn=="true")
		$("#notifica").hide();
		audio.stop();
	});
...
function azioniVelociEsegui(nAzioni,tipo,campo){
    ...
    for(var i=0;i<nAzioni;i++){
		function sto(i){
			setTimeout(function(){
				var percent=i*10/nAzioni;
				if(percent%1==0)
					$("#lavorare").html( $("#lavorare").html()+'#');
...
function aggiugniPiccionPrioritarioDialog(){
    ...
    var h=$(window).height();
    var w=$(window).width(); 
    var hf= $("#piccionDialog").height();
    var wf= hf*1.618;
    $("#piccionDialog").width(wf);
    $("#piccionDialog").css("height","auto");
    $("#piccionDialog").css("top",(h/2-hf/2)+"px");
    $("#piccionDialog").css("left",(w/2-wf/2)+"px");
    
    //$("#piccionDialog").draggable();//TODO MAYBE IT BREAKS EVERYTHING
    $("#piccionDialog").hide();
...
function aggiungiMapButton(){
    ...
    $("#mapButton").click(function(){
        ...
        try{
            ...
            $("#loci").append("Errore imprevisto #555 "+e+"<br />"+js_s);
        }
    });
...
function aggiugniMapDialog(){
    ...
    var h=$(window).height();
    var w=$(window).width(); 
    var hf= $("#mapDialog").height();
    var wf= $("#mapDialog").width();
    $("#mapDialog").width(wf);
    $("#mapDialog").css("top",(h/2-hf/2)+"px");
    $("#mapDialog").css("left",(w/2-wf/2)+"px");
    
    //$("#mapDialog").draggable();//TODO MAYBE IT BREAKS EVERYTHING
    $("#mapDialog").hide();
